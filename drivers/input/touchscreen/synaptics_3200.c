 ...
 *
 * DT2W, double tap to wake support by maxwen, 2013
 */
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (ret < 0) {
        printk(S2W_TAG "set s2w_min_distance failed - %s", buf);
        return count;
    }
	s2w_min_distance = (int)value;
	printk(S2W_TAG "s2w_min_distance=%d", s2w_min_distance);
	return count;
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ret = strict_strtoul(buf, 10, &value);
    if (ret < 0) {
        printk(S2W_TAG "set s2w_register_threshold failed - %s", buf);
        return count;
    }
	s2w_register_threshold = (int)value;
	printk(S2W_TAG "s2w_register_threshold=%d", s2w_register_threshold);
	return count;
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ret = strict_strtoul(buf, 10, &value);
    if (ret < 0) {
        printk(S2W_TAG "set s2w_allow_stroke failed %s", buf);
        return count;
    }
    if (value == 0 || value == 1) {
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ret = strict_strtoul(buf, 10, &value);
    if (ret < 0) {
        printk(S2W_TAG "set s2w_switch failed %s", buf);
        return count;
    }
    if (value == 0 || value == 1) {
	...
	if (ret < 0) {
		...
		if (finger_pressed == 0 /*||
			...
			if ((((ts->finger_count > 0)?1:0) == 0) && s2w_active()) {
				...
				if (s2w_active()){
					...
					if(!mode){
						if(touchDebug)
							printk(S2W_TAG "suspend - ignoring last finger leave");
						return;
					}
		...
		if (ts->pre_finger_data[0][0] < 2 || finger_pressed) {
            ...
            else
            {
              ...
              //left->right
              if ((ts->finger_count == 1) && (scr_suspended == true) && s2w_active()) {
              ...
              //right->left
              } else if ((ts->finger_count == 1) && (scr_suspended == false) && s2w_active()) {
